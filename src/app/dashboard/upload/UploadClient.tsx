      ...
      <AnimatePresence>
        {alert.show && (
          <motion.div 
            ...
            className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[9999] px-12 py-6 bg-[var(--accent)] text-white rounded-[2.5rem] font-bold text-sm tracking-normal shadow-2xl flex items-center gap-6 border-4 border-white/10 italic cursor-pointer"
            onClick={() => setAlert({ message: '', show: false })}
          >
            <ShieldCheck size={20} className="text-white brightness-125" />
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-40 mb-1">Upload Protocol Active</p>
              {alert.message}
            </div>
          </motion.div>
        )}
